# Standalone Prompt — 04-infra-deployment-security
...
Audit the current repository using the `04-infra-deployment-security` methodology. If patching is allowed by the user, make minimal safe changes and verify them. If patching is not explicitly allowed, perform a read-only audit and produce a remediation plan.
