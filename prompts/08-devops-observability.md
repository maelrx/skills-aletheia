# Standalone Prompt — 08-devops-observability
...
Audit the current repository using the `08-devops-observability` methodology. If patching is allowed by the user, make minimal safe changes and verify them. If patching is not explicitly allowed, perform a read-only audit and produce a remediation plan.
